...
export default function Ingresos({
  ...
  const stats = useMemo(() => {
    ...
    if (!list.length) return { total: 0, min: 0, max: 0, avg: 0, count: 0 };
    const amounts = list.map((i) => Number(i.amount) || 0);
  ...
  const cards = [
    ...
    ["Minimo", money(stats.min)],
    ["Maximo", money(stats.max)],
    ["Promedio", money(stats.avg)],
  ];
    ...
    <section className="grid gap-4">
      ...
      {view === "Dashboards" && (
        <div className="card p-6">
          <div className="kicker">INGRESOS</div>
          <h3 className="text-xl font-semibold">Resumen de ingresos</h3>
